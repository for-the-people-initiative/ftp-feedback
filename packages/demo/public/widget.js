var FTPFeedback=(function(h){"use strict";var V=Object.defineProperty;var _=(h,f,y)=>f in h?V(h,f,{enumerable:!0,configurable:!0,writable:!0,value:y}):h[f]=y;var d=(h,f,y)=>_(h,typeof f!="symbol"?f+"":f,y);const f=`
:host {
  --ftp-primary: #f97316;
...
`,y={minMoves:10,minScrolls:3,minKeyPresses:5,minTimeMs:6e4,minClicks:2,minPathVariance:.3,minTypingVariance:500,minSignalsToPass:3},m={cursorBehavior:30,scrolls:8,keyPresses:10,typingPattern:12,clicks:5,time:12,environment:10,backspaces:13},E=7,P=200,T=100;class M{constructor(e,t){d(this,"thresholds");d(this,"signals");d(this,"startTime",0);d(this,"target");d(this,"abortController",null);d(this,"pathPoints",[]);d(this,"pathIndex",0);d(this,"keyTimestamps",[]);d(this,"keyTsIndex",0);this.thresholds={...y,...e},this.target=t||document,this.signals={mouseMoves:0,scrolls:0,keyPresses:0,clicks:0,timeOnPageMs:0,webdriver:!1,hasTouch:!1,screenConsistent:!0,pathAngleVariance:0,pathSamples:0,pathAvgSpeed:0,pathSpeedVariance:0,typingVariance:0,backspaceCount:0,typingSamples:0,typingAvgInterval:0}}start(){this.startTime=Date.now(),this.abortController=new AbortController;const e={signal:this.abortController.signal,passive:!0};let t=0,s=0;this.target.addEventListener("mousemove",i=>{const r=Date.now();if(r-t>50){this.signals.mouseMoves++,t=r;const n=i;this.addPathPoint(n.clientX,n.clientY,r)}},e),this.target.addEventListener("touchmove",i=>{const r=Date.now();if(r-t>50){this.signals.mouseMoves++,t=r;const n=i;n.touches.length>0&&this.addPathPoint(n.touches[0].clientX,n.touches[0].clientY,r)}},e),this.target.addEventListener("scroll",()=>{const i=Date.now();i-s>200&&(this.signals.scrolls++,s=i)},e),window.addEventListener("scroll",()=>{const i=Date.now();i-s>200&&(this.signals.scrolls++,s=i)},e),this.target.addEventListener("keydown",i=>{this.signals.keyPresses++;const r=Date.now(),n=i;(n.key==="Backspace"||n.key==="Delete")&&this.signals.backspaceCount++,this.keyTimestamps.length<T?this.keyTimestamps.push(r):this.keyTimestamps[this.keyTsIndex%T]=r,this.keyTsIndex++},e),this.target.addEventListener("click",()=>{this.signals.clicks++},e),this.target.addEventListener("touchstart",()=>{this.signals.clicks++},e),this.signals.webdriver=!!navigator.webdriver,this.signals.hasTouch="ontouchstart"in window||navigator.maxTouchPoints>0,this.signals.screenConsistent=this.checkScreenConsistency()}evaluate(){this.signals.timeOnPageMs=Date.now()-this.startTime,this.analyzePath(),this.analyzeTyping();const e=this.thresholds,t=this.signals,s=t.mouseMoves>=e.minMoves&&t.pathAngleVariance>=e.minPathVariance&&t.pathSamples>=5,i=t.scrolls>=e.minScrolls,r=t.keyPresses>=e.minKeyPresses,n=t.clicks>=e.minClicks,a=t.timeOnPageMs>=e.minTimeMs,l=t.typingVariance>=e.minTypingVariance&&t.typingSamples>=5||t.backspaceCount>=1,c=!t.webdriver&&t.screenConsistent,g={cursorBehavior:{proven:s,value:t.mouseMoves,threshold:e.minMoves,label:"Cursor behavior"},scrolls:{proven:i,value:t.scrolls,threshold:e.minScrolls,label:"Scrolls"},keyPresses:{proven:r,value:t.keyPresses,threshold:e.minKeyPresses,label:"Key presses"},clicks:{proven:n,value:t.clicks,threshold:e.minClicks,label:"Clicks"},time:{proven:a,value:t.timeOnPageMs,threshold:e.minTimeMs,label:"Time on page"},typingPattern:{proven:l,value:t.typingVariance,threshold:e.minTypingVariance,label:"Typing rhythm"},environment:{proven:c,value:c?1:0,threshold:1,label:"Environment"}},p=Object.values(g).filter(O=>O.proven).length,x=Math.min(Math.round(p/e.minSignalsToPass*100),100),u=p>=e.minSignalsToPass,b=Math.min(t.mouseMoves/e.minMoves,1),v=this.cursorPathScore(),$=b*v,C=t.typingSamples>=3?Math.min(t.typingVariance/e.minTypingVariance,1):0,L=Math.min(t.backspaceCount/2,1),B=Math.round($*m.cursorBehavior+Math.min(t.scrolls/e.minScrolls,1)*m.scrolls+Math.min(t.keyPresses/e.minKeyPresses,1)*m.keyPresses+C*m.typingPattern+Math.min(t.clicks/e.minClicks,1)*m.clicks+Math.min(t.timeOnPageMs/e.minTimeMs,1)*m.time+this.envScore()*m.environment+L*m.backspaces);return{score:x,passed:u,provenCount:p,totalSignals:E,minRequired:e.minSignalsToPass,confidenceScore:B,breakdown:g,signals:{...this.signals}}}evaluateWith(e){const t={...this.signals},s=this.startTime;Object.assign(this.signals,e),e.timeOnPageMs!==void 0&&(this.startTime=Date.now()-e.timeOnPageMs);const i=this.evaluate();return this.signals=t,this.startTime=s,i}getThresholds(){return{...this.thresholds}}setThresholds(e){Object.assign(this.thresholds,e)}destroy(){var e;(e=this.abortController)==null||e.abort(),this.abortController=null,this.pathPoints=[],this.keyTimestamps=[]}analyzeTyping(){const e=this.keyTimestamps;if(this.signals.typingSamples=e.length,e.length<3){this.signals.typingVariance=0,this.signals.typingAvgInterval=0;return}const t=[...e].sort((i,r)=>i-r),s=[];for(let i=1;i<t.length;i++){const r=t[i]-t[i-1];r<5e3&&s.push(r)}if(s.length<2){this.signals.typingVariance=0,this.signals.typingAvgInterval=0;return}this.signals.typingAvgInterval=s.reduce((i,r)=>i+r,0)/s.length,this.signals.typingVariance=this.variance(s)}addPathPoint(e,t,s){this.pathPoints.length<P?this.pathPoints.push({x:e,y:t,t:s}):this.pathPoints[this.pathIndex%P]={x:e,y:t,t:s},this.pathIndex++}analyzePath(){const e=this.pathPoints;if(this.signals.pathSamples=e.length,e.length<3){this.signals.pathAngleVariance=0,this.signals.pathAvgSpeed=0,this.signals.pathSpeedVariance=0;return}const t=[],s=[];for(let i=1;i<e.length;i++){const r=e[i].x-e[i-1].x,n=e[i].y-e[i-1].y,a=e[i].t-e[i-1].t,l=Math.sqrt(r*r+n*n);if(a>0&&s.push(l/a),i>=2){const c=e[i-1].x-e[i-2].x,g=e[i-1].y-e[i-2].y,p=Math.atan2(g,c);let u=Math.atan2(n,r)-p;for(;u>Math.PI;)u-=2*Math.PI;for(;u<-Math.PI;)u+=2*Math.PI;t.push(u)}}this.signals.pathAngleVariance=this.variance(t),s.length>0&&(this.signals.pathAvgSpeed=s.reduce((i,r)=>i+r,0)/s.length,this.signals.pathSpeedVariance=this.variance(s))}cursorPathScore(){if(this.pathPoints.length<5)return .5;let t=0;const s=this.signals.pathAngleVariance;t+=Math.min(s/this.thresholds.minPathVariance,1)*.6;const i=this.signals.pathSpeedVariance,r=i>.01?Math.min(i/.1,1):0;return t+=r*.4,Math.min(t,1)}envScore(){let e=1;return this.signals.webdriver&&(e-=.6),this.signals.screenConsistent||(e-=.3),this.signals.hasTouch&&this.signals.mouseMoves===0&&this.signals.timeOnPageMs>1e4&&(e-=.1),Math.max(0,e)}checkScreenConsistency(){const{innerWidth:e,innerHeight:t,screen:s}=window;return!(e===0||t===0||s.width===0||s.height===0||e>s.width+50||t>s.height+50)}variance(e){if(e.length<2)return 0;const t=e.reduce((i,r)=>i+r,0)/e.length;return e.map(i=>(i-t)**2).reduce((i,r)=>i+r,0)/e.length}}const w={bug:{totalSteps:4,steps:[{key:"title",title:"What happened?",subtitle:"Give a brief title for the bug",type:"input",required:!0,placeholder:"e.g. Button doesn't respond when clicked"},{key:"reproduction",title:"Steps to reproduce",subtitle:"What were you doing when this happened?",type:"textarea",required:!1,placeholder:"I clicked on... then I..."},{key:"expected",title:"Expected vs actual",subtitle:"What should have happened instead?",type:"textarea",required:!1,placeholder:"I expected... but instead..."},{key:"severity",title:"How severe is this?",subtitle:"Pick the option that best describes the impact",type:"severity",required:!0}]},suggestion:{totalSteps:3,steps:[{key:"title",title:"What's your idea?",subtitle:"A short title for your suggestion",type:"input",required:!0,placeholder:"e.g. Add dark mode support"},{key:"description",title:"Tell us more",subtitle:"Describe your idea in detail",type:"textarea",required:!1,placeholder:"It would be great if..."},{key:"motivation",title:"Why does it matter?",subtitle:"Help us understand the value (optional)",type:"textarea",required:!1,placeholder:"This would help because..."}]},question:{totalSteps:2,steps:[{key:"title",title:"What's your question?",subtitle:"Ask away — no question is too small",type:"textarea",required:!0,placeholder:"How do I..."},{key:"context",title:"Where are you stuck?",subtitle:"Share the page or context (optional)",type:"input",required:!1,placeholder:"URL or description",defaultValue:()=>window.location.href}]}},A=[{value:"blocking",label:"Blocking",icon:"🔴",desc:"Can't continue"},{value:"major",label:"Major",icon:"🟠",desc:"Significant issue"},{value:"minor",label:"Minor",icon:"🟡",desc:"Small annoyance"},{value:"cosmetic",label:"Cosmetic",icon:"🟢",desc:"Visual only"}],k="ftp-feedback-draft",I="https://ftp-feedback-api.onrender.com";class z extends HTMLElement{constructor(){super();d(this,"shadow");d(this,"config");d(this,"isOpen",!1);d(this,"wizard",{category:null,step:0,data:{}});d(this,"submitting",!1);d(this,"trust",null);this.shadow=this.attachShadow({mode:"open"}),this.config={appId:"",apiUrl:I,position:"bottom-right",theme:"light",categories:["bug","suggestion","question"],user:{}}}static get observedAttributes(){return["app-id","api-url","position","theme","categories","user-id","user-email","branding","trust-min-moves","trust-min-scrolls","trust-min-keys","trust-min-time","trust-min-clicks"]}connectedCallback(){this.config.appId=this.getAttribute("app-id")||this.config.appId,this.config.apiUrl=this.getAttribute("api-url")||this.config.apiUrl,this.config.position=this.getAttribute("position")||this.config.position,this.config.theme=this.getAttribute("theme")||this.config.theme,this.config.user.id=this.getAttribute("user-id")||void 0,this.config.user.email=this.getAttribute("user-email")||void 0;const t=this.getAttribute("categories");t&&(this.config.categories=t.split(",").map(c=>c.trim())),this.config.theme!=="light"&&this.setAttribute("theme",this.config.theme);const s={},i=this.getAttribute("trust-min-moves"),r=this.getAttribute("trust-min-scrolls"),n=this.getAttribute("trust-min-keys"),a=this.getAttribute("trust-min-time"),l=this.getAttribute("trust-min-clicks");i&&(s.minMoves=parseInt(i)),r&&(s.minScrolls=parseInt(r)),n&&(s.minKeyPresses=parseInt(n)),a&&(s.minTimeMs=parseInt(a)),l&&(s.minClicks=parseInt(l)),this.trust=new M(s),this.trust.start(),this.loadDraft(),this.render()}disconnectedCallback(){var t;(t=this.trust)==null||t.destroy()}configure(t){Object.assign(this.config,t),t.theme&&this.setAttribute("theme",t.theme),this.render()}open(){this.isOpen=!0,this.render()}close(){this.isOpen=!1,this.submitting=!1,this.render()}resetWizard(){this.wizard={category:null,step:0,data:{}},this.clearDraft()}saveDraft(){try{sessionStorage.setItem(k,JSON.stringify(this.wizard))}catch{}}loadDraft(){try{const t=sessionStorage.getItem(k);if(t){const s=JSON.parse(t);s.category&&(this.wizard=s)}}catch{}}clearDraft(){try{sessionStorage.removeItem(k)}catch{}}get flow(){return this.wizard.category?w[this.wizard.category]:null}get totalStepsWithConfirm(){return this.flow?this.flow.totalSteps+1:0}get currentFlowStep(){var t;return(t=this.flow)==null?void 0:t.steps[this.wizard.step-1]}get isConfirmStep(){return this.flow&&this.wizard.step===this.flow.totalSteps+1}canProceed(){var i;const t=this.currentFlowStep;return!t||!t.required?!0:!!((i=this.wizard.data[t.key])==null?void 0:i.trim())}render(){const t=this.config.position;this.shadow.innerHTML=`
      <style>${f}</style>
    ...
    `}renderFlowStep(){var n;const t=this.currentFlowStep,s=this.wizard.data[t.key]??(((n=t.defaultValue)==null?void 0:n.call(t))||"");t.required;const i=this.wizard.step===this.flow.totalSteps;let r="";return t.type==="input"?r=`<input type="text" id="stepInput" placeholder="${t.placeholder||""}" value="${this.escAttr(s)}" maxlength="200">`:t.type==="textarea"?r=`<textarea id="stepInput" placeholder="${t.placeholder||""}" rows="4">${this.escHtml(s)}</textarea>`:t.type==="severity"&&(r=`<div class="severity-grid">${A.map(a=>`
        <button class="severity-btn ${s===a.value?"active":""}" data-sev="${a.value}">
    ...
    `}categoryLabel(t){return t==="bug"?"🐛 Bug Report":t==="suggestion"?"💡 Suggestion":"❓ Question"}bindEvents(){var s,i,r,n,a;(s=this.shadow.getElementById("trigger"))==null||s.addEventListener("click",()=>this.isOpen?this.close():this.open()),(i=this.shadow.getElementById("close"))==null||i.addEventListener("click",()=>this.close()),this.shadow.querySelectorAll(".category-card").forEach(l=>{l.addEventListener("click",()=>{const c=l.dataset.cat;this.wizard.category=c,this.wizard.step=1,w[c].steps.forEach(p=>{p.defaultValue&&!this.wizard.data[p.key]&&(this.wizard.data[p.key]=p.defaultValue())}),this.saveDraft(),this.render()})});const t=this.shadow.getElementById("stepInput");t&&(t.addEventListener("input",()=>{const l=this.currentFlowStep;l&&(this.wizard.data[l.key]=t.value,this.saveDraft())}),requestAnimationFrame(()=>t.focus())),this.shadow.querySelectorAll(".severity-btn").forEach(l=>{l.addEventListener("click",()=>{const c=l.dataset.sev;this.wizard.data.severity=c,this.saveDraft(),this.shadow.querySelectorAll(".severity-btn").forEach(p=>p.classList.remove("active")),l.classList.add("active");const g=this.shadow.getElementById("nextBtn");g&&(g.disabled=!1)})}),(r=this.shadow.getElementById("backBtn"))==null||r.addEventListener("click",()=>{this.wizard.step<=1?(this.wizard.step=0,this.wizard.category=null):this.wizard.step--,this.saveDraft(),this.render()}),(n=this.shadow.getElementById("nextBtn"))==null||n.addEventListener("click",()=>{this.wizard.step++,this.saveDraft(),this.render()}),(a=this.shadow.getElementById("submitBtn"))==null||a.addEventListener("click",()=>this.submit())}async submit(){var c,g,p,x,u;if(this.submitting)return;this.submitting=!0,this.render();const t=this.wizard.category,s=this.wizard.data;let i={},r={};t==="bug"?(i={reproduction:s.reproduction||"",expected:s.expected||""},r={severity:s.severity||""}):t==="suggestion"?i={description:s.description||"",motivation:s.motivation||""}:i={context:s.context||""};const n=this.collectMetadata(),a=(c=this.trust)==null?void 0:c.evaluate(),l={type:t,title:s.title||"",body:JSON.stringify(i),user_id:this.config.user.id,user_email:this.config.user.email,page_url:window.location.href,route:window.location.pathname,user_agent:navigator.userAgent,viewport:`${window.innerWidth}x${window.innerHeight}`,metadata:{...n,...r,trust_score:a==null?void 0:a.score,trust_passed:a==null?void 0:a.passed,trust_signals:a==null?void 0:a.signals}};try{const b=await fetch(`${this.config.apiUrl}/v1/feedback`,{method:"POST",headers:{"Content-Type":"application/json","X-App-Id":this.config.appId},body:JSON.stringify(l)});if(!b.ok){const v=await b.json().catch(()=>({}));throw new Error(v.error||`HTTP ${b.status}`)}(p=(g=this.config).onSubmit)==null||p.call(g,l),this.clearDraft(),this.showSuccess()}catch(b){this.submitting=!1,this.render();const v=this.shadow.getElementById("errorMsg");v&&(v.textContent=b.message||"Failed to submit",v.style.display="block"),(u=(x=this.config).onError)==null||u.call(x,b)}}collectMetadata(){var l,c;const t=navigator.userAgent,s=navigator;let i="Unknown";/Windows NT 10/.test(t)?i="Windows 10/11":/Windows NT/.test(t)?i="Windows":/Mac OS X (\d+[._]\d+)/.test(t)?i=`macOS ${RegExp.$1.replace("_",".")}`:/iPhone OS (\d+[._]\d+)/.test(t)?i=`iOS ${RegExp.$1.replace("_",".")}`:/Android (\d+(\.\d+)?)/.test(t)?i=`Android ${RegExp.$1}`:/Linux/.test(t)&&(i="Linux");let r="Unknown";/Edg\/(\d+)/.test(t)?r=`Edge ${RegExp.$1}`:/Chrome\/(\d+)/.test(t)?r=`Chrome ${RegExp.$1}`:/Safari\/(\d+)/.test(t)&&/Version\/(\d+(\.\d+)?)/.test(t)?r=`Safari ${RegExp.$1}`:/Firefox\/(\d+)/.test(t)&&(r=`Firefox ${RegExp.$1}`);const n=/Mobi|Android.*Mobile|iPhone/.test(t),a=s.connection||s.mozConnection||s.webkitConnection;return{device_type:n?"mobile":"desktop",os:i,browser:r,screen_resolution:`${screen.width}x${screen.height}`,language:navigator.language,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,connection_type:(a==null?void 0:a.effectiveType)||null,color_scheme:(c=(l=window.matchMedia)==null?void 0:l.call(window,"(prefers-color-scheme: dark)"))!=null&&c.matches?"dark":"light",pixel_ratio:window.devicePixelRatio,online:navigator.onLine,referrer:document.referrer||null}}showSuccess(){this.wizard={category:null,step:0,data:{}};const t=this.shadow.getElementById("formBody");t&&(t.innerHTML=`
        <div class="success">
          <div class="check">✅</div>
          <h3>Thank you!</h3>
          <p>Your feedback has been submitted.</p>
        </div>
      `),setTimeout(()=>this.close(),2500)}escHtml(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}escAttr(t){return this.escHtml(t).replace(/"/g,"&quot;")}}customElements.get("ftp-feedback")||customElements.define("ftp-feedback",z);const S={_el:null,init(o){var t,s,i;(t=document.querySelector("ftp-feedback"))==null||t.remove();const e=document.createElement("ftp-feedback");return e.setAttribute("app-id",o.appId),o.apiUrl&&e.setAttribute("api-url",o.apiUrl),o.position&&e.setAttribute("position",o.position),o.theme&&e.setAttribute("theme",o.theme),o.categories&&e.setAttribute("categories",o.categories.join(",")),(s=o.user)!=null&&s.id&&e.setAttribute("user-id",o.user.id),(i=o.user)!=null&&i.email&&e.setAttribute("user-email",o.user.email),document.body.appendChild(e),requestAnimationFrame(()=>{e.configure({onSubmit:o.onSubmit,onError:o.onError})}),this._el=e,e},open(){var o;(o=this._el)==null||o.open()},close(){var o;(o=this._el)==null||o.close()}};return(function(){const e=document.currentScript||document.querySelector("script[data-app-id]");if(e&&e instanceof HTMLScriptElement){const t=e.getAttribute("data-app-id");if(t){const s=()=>{var i;S.init({appId:t,apiUrl:e.getAttribute("data-api-url")||void 0,position:e.getAttribute("data-position")||void 0,theme:e.getAttribute("data-theme")||void 0,categories:(i=e.getAttribute("data-categories"))==null?void 0:i.split(",").map(r=>r.trim()),user:{id:e.getAttribute("data-user-id")||void 0,email:e.getAttribute("data-user-email")||void 0}})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",s):s()}}})(),h.FTPFeedback=S,h.FTPFeedbackElement=z,h.TrustScore=M,h.default=S,Object.defineProperties(h,{__esModule:{value:!0},[Symbol.toStringTag]:{value:"Module"}}),h})({});
